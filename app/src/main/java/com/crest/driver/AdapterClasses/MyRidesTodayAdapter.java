...
public class MyRidesTodayAdapter extends RecyclerView.Adapter<MyRidesTodayAdapter.MyView> {

    List<MyRidesModel> list;
    Context context;

    public MyRidesTodayAdapter(List<MyRidesModel> list) {
        this.list = list;
    }

    @Override
    public MyRidesTodayAdapter.MyView onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.may_rides_list_items, parent, false);
        context = parent.getContext();
        return new MyRidesTodayAdapter.MyView(view);
    }

    @Override
    public void onBindViewHolder(MyRidesTodayAdapter.MyView holder, final int position) {
